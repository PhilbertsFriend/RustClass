...
impl ThreadPool {
        ...
        F: FnOnce() + Send + 'static,
    {
        ...
        let newJob = Box::new(f);
        self.sender.send(Message::NewJob(newJob)).unwrap();
    }
}
...
impl Drop for ThreadPool {
    fn drop(&mut self) {
        ...
        println!("Terminating Pool of Workers");
...
impl Worker {
    ...
    fn new(id: usize, receiver: Arc<Mutex<mpsc::Receiver<Message>>>) -> Worker {
        ...
        let newThread = thread::spawn(move ||
            loop {
                let rec = receiver.lock().unwrap().recv().unwrap();

                match rec {
                    Message::NewJob(newJob) => {
                        println!("Thread {} starting job.", id);
                        newJob();
                    }
